  ...
  public function data(Request $request)
  {
    ...
    $rows = $users->map(function ($user) {
      ...
      if (auth()->user()->can('delete users') && $user->id !== auth()->id()) {
        $actions .= '<form method="POST" action="' . route('admin.users.destroy', $user) . '" class="d-inline swal-delete-form">'
          . csrf_field() . method_field('DELETE')
          . '<button type="button" class="btn btn-sm btn-icon btn-outline-danger swal-delete-btn"><i class="bi bi-trash"></i></button>'
          . '</form>';
      }
